...
string dir_name(const string s, const string d) {
    if (s.empty()) return {};
    vector<string> res;
    string strs = s + d;
...
int main (int argc,char *argv[])
{
    ...
    string dir_path = dir_name(output_file, "/"); 
    const char * output_dir = dir_path.c_str();
    ...
    for (size_t i = 0; i < input_images.size() && ok; i++) {
        ...
        if (D) {
            output_image = ApplyTransform(input_image, D, interpolate, output_dir);
