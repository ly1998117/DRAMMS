...
void ScaleImage(Image* scaled, const Image* image, float slope, float inter)
{
    ...
    // rescale intensities
	float v; // Yangming added on 2/2/2014    
    for (int k = 0; k < image->region.nz; k++) {
        for (int j = 0; j < image->region.ny; j++) {
            for (int i = 0; i < image->region.nx; i++) {
                for (int n = 0; n < N; n++) {
					v = image->get(i, j, k, n) * slope; 
					if ( v == 0 )
						scaled->set(i, j, k, n, 0);  // Yangming added on 2/2014, to avoid the background voxels having intensity 0 being assigned into foreground when the minimum intensity in the image is negative.
					else
						scaled->set(i, j, k, n, v + inter);
                }
            }
        }
    }
...
int FindAdaptiveThreshold(float* histo, int nbins)
{
	int ind;
	int ind_bin=12;
	while (histo[ind_bin]>0.4) {
		ind_bin--;
	} //added on 3/16/2014, to avoid the default threshold 12 from cutting off more than 40% of the voxels (in oneextreme case (BLSA_0601_17-5_02_T1_LPS_N4.nii.gz), threshold 12 will cut off >90% of the content, leaving only skull boundaries, while adding this constrain will reduce the default threshold to intenstiy 3, which only cuts off ~35% of the content


	int maxPossible=28;  // the biggest possible threshold is the 28-th bin by default
	...
	for (ind = 28; ind < nbins; ind=ind+4) // starting from 2/2/2014, now the biggest possible threshold is dynamic (not fixed at 28 any more)
		{
...
// ---------------------------------------------------------------------------
Image::Transform MultiplyTransform(const Image::Transform& T, float factor)
{
	Image::Transform mT;
	for (int r=0; r<4; r++)
	  for (int c=0; c<4; c++) 
		mT.m[r][c] = T.m[r][c] * factor;
		
	return mT;
}

// ---------------------------------------------------------------------------
Image* MultiplyTransform(const Image* D, float factor)
{
    assert(D != NULL);
	...
	Image* mD = new Image(nx, ny, nz, D->hdr.datatype, N, D->imgfmt);
    if (mD == NULL) return NULL;
    mD->CopyTransform(D);
    mD->CopyDataScaling(D);
    mD->CopyMetaData(D);

	for (int k = 0; k < nz; k++) {
            for (int i = 0; i < nx; i++) {
                for (int j = 0; j < ny; j++) {
					mD->img.v3[k][i][j].x = D->img.v3[k][i][j].x * factor;
					mD->img.v3[k][i][j].y = D->img.v3[k][i][j].y * factor;
					mD->img.v3[k][i][j].z = D->img.v3[k][i][j].z * factor;
				}
            }
		}
...
Image::Transform DivideTransform(const Image::Transform& T, float factor)
{
	...
	for (int r=0; r<4; r++)
	  for (int c=0; c<4; c++) 
		dT.m[r][c] = T.m[r][c] / factor;
		
	return dT;
}

// ---------------------------------------------------------------------------
Image* DivideTransform(const Image* D, float factor)
{
    assert(D != NULL);
	...
	Image* dD = new Image(nx, ny, nz, D->hdr.datatype, N, D->imgfmt);
    if (dD == NULL) return NULL;
    dD->CopyTransform(D);
    dD->CopyDataScaling(D);
    dD->CopyMetaData(D);

	for (int k = 0; k < nz; k++) {
            for (int i = 0; i < nx; i++) {
                for (int j = 0; j < ny; j++) {
					dD->img.v3[k][i][j].x = D->img.v3[k][i][j].x / factor;
					dD->img.v3[k][i][j].y = D->img.v3[k][i][j].y / factor;
					dD->img.v3[k][i][j].z = D->img.v3[k][i][j].z / factor;
				}
            }
		}
...
Image* ApplyTransform(const Image* image, const Image* deffield, int interpolate)
{
    ...
    Image* warpedimage = new Image(deffield->region.nx,
                                   ...
                                   image->hdr.datatype, 1,
                                   deffield->imgfmt);
    Image* deffild_x = new Image(deffield->region.nx,
                                   deffield->region.ny,
                                   deffield->region.nz,
                                   image->hdr.datatype, 1,
                                   deffield->imgfmt);
    Image* deffild_y = new Image(deffield->region.nx,
                                   deffield->region.ny,
                                   deffield->region.nz,
                                   image->hdr.datatype, 1,
                                   deffield->imgfmt);
    Image* deffild_z = new Image(deffield->region.nx,
                                   deffield->region.ny,
                                   deffield->region.nz,
                                   image->hdr.datatype, 1,
                                   deffield->imgfmt);
    ...
    const int y_size_out = deffield->region.ny;
    const int z_size_out = deffield->region.nz;
    cout << "df shape: " << x_size_out << y_size_out << z_size_out << endl;
    float x, y, z, value;
    vector<float> v;
    Image *DF = new Image(deffield->region.nx, deffield->region.ny, deffield->region.nz,
                              DT_FLOAT, 3, deffield->imgfmt);
    Sequence DFx, DFy, DFz;
    for (int k = 0; k < z_size_out; k++) {
        for (int j = 0; j < y_size_out; j++) {
            for (int i = 0; i < x_size_out; i++) {
		...
		// cout << "TYPE"  << deffield->imgfmt <<"df inidex: " << "[i:"<< i << "j:" << j << "K:" << k << "] value: " << v[0]<< " " << v[1] <<" "<< v[2] << endl;
		// cout << "img get: " << "[i:"<< i << "j:" << j << "K:" << k << "] value: " << image->get(i,j,k) << endl;
		// cout << "img value: " << "[i:"<< i << "j:" << j << "K:" << k << "] value: " << image->value(i,j,k) << endl;
		x = static_cast<float>(i) + v[0];
		deffild_x->set(i,j,k, v[0]);
		deffild_y->set(i,j,k, v[1]);
		deffild_z->set(i,j,k, v[2]);
    ...
    DFx.push_back(deffild_x);
    DFy.push_back(deffild_y);
    DFz.push_back(deffild_z);
    WriteSequence("DFx.nii.gz", DFx);
    WriteSequence("DFy.nii.gz", DFy);
    WriteSequence("DFz.nii.gz", DFz);
    return warpedimage;
}

// ---------------------------------------------------------------------------
Image* ApplyTransform(const Image* image, const Image* deffield, int interpolate, const char * output_dir)
{
    // check input images
    if (image->hdr.dim[5] > 1) {
        BASIS_THROW(invalid_argument, "Only scalar images can be warped by this program!");
    ...
    WriteSequence((string(output_dir) + string("DFx.nii.gz")).c_str(), DFx);
    WriteSequence((string(output_dir) + string("DFy.nii.gz")).c_str(), DFy);
    WriteSequence((string(output_dir) + string("DFz.nii.gz")).c_str(), DFz);
    return warpedimage;
}
